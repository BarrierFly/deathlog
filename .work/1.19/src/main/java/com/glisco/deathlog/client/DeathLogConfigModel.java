...
public class DeathLogConfigModel {
    ...
    private static final Path CONFIG_PATH = FabricLoader.getInstance().getConfigDir().resolve("deathlog.json");
    ...
    public static DeathLogConfigModel load() {
        if (Files.exists(CONFIG_PATH)) {
            try {
                return GSON.fromJson(Files.readString(CONFIG_PATH), DeathLogConfigModel.class);
            } catch (IOException e) {
