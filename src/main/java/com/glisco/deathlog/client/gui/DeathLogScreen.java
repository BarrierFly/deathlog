        ...
        if (deathList.getSelectedOrNull() != null) {
            ...
            if (info.isPartial() && this.storage instanceof RemoteDeathLogStorage remote) {
                ...
                for (int i = 0; i < left.size(); i++)
                    textRenderer.draw(matrices, left.get(i), originX, detailTop + 14 * i, 0xFFFFFF);
                final var right = info.getRightColumnText();
                for (int i = 0; i < right.size(); i++)
                    drawRightColumnText(matrices, right.get(i), originX + 100, detailTop + 14 * i);
                ...
                if (hoveredStack != null) {
                    ...
                    tooltip.add(Text.translatable(this.client.player.isCreative() ? "text.deathlog.action.give_item.spawn" : "text.deathlog.action.give_item.copy_give").formatted(net.minecraft.util.Formatting.GRAY));
