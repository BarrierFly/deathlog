    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        if (deathList.getSelectedOrNull() != null) {
            ...
            if (info.isPartial() && this.storage instanceof RemoteDeathLogStorage remote) {
                ...
                context.drawTexture(texture -> RenderLayer.getGuiTextured(texture), INVENTORY_TEXTURE, originX - 8, originY - 83, 0, 0, 210, 107, 210, 107);
                hoveredStack = null;
    ...
    public boolean mouseClicked(double mx, double my, int button) {
        if (hoveredStack != null && button == 2) {
            if (client.player.isCreative()) {
                ...
                var encodedNbt = hoveredStack.toNbt(client.world.getRegistryManager());
                if (encodedNbt instanceof NbtCompound compoundNbt) nbt = compoundNbt;
                cmd.append(nbt);
